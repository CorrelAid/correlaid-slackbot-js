...
module.exports.handler = async function(event, context) {
    ...
    if (command && text) {
        ...
        if (commandArgs.subcommand == 'details') {
            // post telko details
            const telkoDetails = utils.readFromFile(
                './src/messages/telko_details.txt'
            )
            return responses.buildSlashCommandResponse(
                telkoDetails,
                'in_channel'
            )
        } else if (commandArgs.subcommand == 'poll') {
            const pollMessage = utils.readFromFile(
                './src/messages/telko_poll.txt'
            )
            return responses.buildSlashCommandResponse(
                pollMessage,
                'in_channel'
            ...
            // unknown subcommand
            const errorMessage =
                'Unknown subcommand. Available: `poll` and `details`'
            return responses.buildSlashCommandResponse(
                errorMessage,
                'ephemeral'
            )
